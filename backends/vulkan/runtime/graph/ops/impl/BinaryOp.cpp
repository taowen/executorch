...
void add_binary_op_node(
  ...
  const bool lhs_is_tensor_like = graph.val_is_tensor(in1) || graph.val_is_tref(in1);
  const bool rhs_is_tensor_like = graph.val_is_tensor(in2) || graph.val_is_tref(in2);
