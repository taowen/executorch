...
def _default_pte() -> Path:
    path = Path("artifacts/pte/qwen3_5_0_8b_vulkan_fp32_statefix_main.pte")
    if path.is_file():
        return path.resolve()
    candidates = sorted(glob.glob("artifacts/pte/qwen3_5_0_8b_vulkan*.pte"))
    if not candidates:
        raise RuntimeError("Cannot find qwen3_5_0_8b Vulkan PTE under artifacts/pte")
...
def _default_tokenizer() -> Path:
    candidates = sorted(
        glob.glob(
            str(
                Path.home()
                / ".cache/huggingface/hub/models--Qwen--Qwen3.5-0.8B/snapshots/*/tokenizer.json"
            )
        )
    )
    if not candidates:
        raise RuntimeError("Cannot find Qwen3.5-0.8B tokenizer.json in HF cache")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--prefill-chunk-size", type=int, default=1)
    parser.add_argument(
        "--stream",
