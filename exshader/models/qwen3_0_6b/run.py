...
def _default_pte() -> Path:
    candidates = sorted(glob.glob("artifacts/pte/qwen3_0_6b_vulkan*.pte"))
    if not candidates:
        raise RuntimeError("Cannot find qwen3_0_6b Vulkan PTE under artifacts/pte")
...
def _default_tokenizer() -> Path:
    ...
    if not candidates:
        raise RuntimeError("Cannot find Qwen3-0.6B tokenizer.json in HF cache")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--prompt", default="Write a short poem about Vulkan.")
